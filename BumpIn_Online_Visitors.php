<?php
/*
	Plugin Name: BumpIn Real Time Visitors
...
function render_bumpin_live_feed( $opt, $before_title, $after_title) {
...
function widget_bumpin_live_feed_control( $args=null ) {
...
?>
		<p style="text-align:justify;">It seems you are already using BumpIn SocialBar widgets. </p>
                <p style="text-align:justify; color:red;">If you dont see the "Visitors" tab in the SocialBar at your site, Please follow the steps below.</p>
		<span style="text-align:justify;">
                    ...
                    <ol>
                        ...
                        <li>Click on the "Edit Socialbar" link, click the "Real Time Visitors" app in the "Select Applications" tab.</li>
                        <li>Hit the "Update" button and refresh your website to start using Online Visitors in the BumpIn SocialBar.</li>
                   </ol>
		</span>
		<p>
...
?>
	...
	<p style="text-align:justify;">
		...
		To get the code for the bar <a href = "http://socialbar.ticketmy.com/checkout?wordpress=true&plugin=true&live_feed=true" target = "_blank">Click Here</a>
	</p>
	<p>
		<textarea style="width: 590px; height: 150px;" id="bumpin_code" name="bumpin_code"><?php echo $bumpin_code; ?></textarea>
		<input type="hidden" id="submit_code" name="submit_code" value="1" />
...
function widget_bumpin_live_feed($args) {
	extract($args);
	$options = get_option('BumpIn');
	echo $before_widget;
	render_bumpin_live_feed($options,$before_title,$after_title);
  	echo $after_widget;
}

function bumpin_live_feed_init()
{
	register_sidebar_widget(__('Real Time Visitors'), 'widget_bumpin_live_feed');
	register_widget_control('Real Time Visitors','widget_bumpin_live_feed_control',600,600);    /* width, height*/
}
add_action("plugins_loaded", "bumpin_live_feed_init");
?>
